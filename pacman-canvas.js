...
/* ----- Global Variables ---------------------------------------- */
	...
	function Game() {
		...
		this.ghostSpeedNormal = (this.level > 4 ? 4 : 3);	// global default for ghost speed
		...
		this.getLevelTitle = function() {
			switch(this.level) {
				case 2:
					return '"Dollar Crisis begins"';
                    // activate chase / scatter switching
				case 3:
					return '"Quantitative Easing starts"';
                    // Inky starts leaving the ghost house
				case 4:
					return '"Hyperinflation begins"';
		...
		this.init = function (state) {
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost("pinky",7,5,'img/dollarbill.svg',2,2);
				inky = new Ghost("inky",8,5,'img/dollarbill.svg',13,11);
				blinky = new Ghost("blinky",9,5,'img/dollarbill.svg',13,0);
				clyde = new Ghost("clyde",10,5,'img/dollarbill.svg',2,11);
			}
			else {
	...
	function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
		...
		this.dazzleImg.src = 'img/pile-of-dung.svg';
		this.dazzleImg2 = new Image();
		this.dazzleImg2.src = 'img/dollarbill.svg';
		...
		this.draw = function (context) {					
			if (this.dead) {
				...
				if (pacman.beastModeTimer < 50 && pacman.beastModeTimer % 8 > 1) {
					context.drawImage(this.dazzleImg2, this.posX, this.posY, 2*this.radius, 2*this.radius+incGhostSize);
				} else {
					context.drawImage(this.dazzleImg, this.posX, this.posY, 2*this.radius, 2*this.radius);
				}
			}
			else context.drawImage(this.image, this.posX, this.posY, 2*this.radius, 2*this.radius+incGhostSize);
		}
			...
			$.each(game.map.posY, function(i, item) {
			   ...
			   $.each(this.posX, function() { 
				   if (this.type == "pill") {
					...
					var yyy=10;
					var h= '20BF';
					context.fillText(String.fromCharCode(parseInt(h, 16)), game.toPixelPos(this.col-1)+pacman.radius-xxx, game.toPixelPos(dotPosY-1)+pacman.radius+yyy);
